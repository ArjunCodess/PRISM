...
5. Trains persistence, median, Ridge, MAE residual XGBoost, collapse and warning classifiers, hurdle mix, conformal intervals, a 10-model bootstrap for disagreement, then runs snapshot-versus-history ablation, multi-horizon evaluation, abstention coverage, failure clustering, and SHAP contrast.
6. Evaluates the untouched local test set, exports SHAP explanations and six curated cases, and saves all frozen artifacts (`prism-0.3.0`).
...
Hurdle training lives in `ml/src/hurdle.py`. Geometry features live in `ml/src/features.py`. Abstention rules live in `ml/src/abstention.py`.
