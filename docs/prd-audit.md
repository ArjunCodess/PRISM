...
- Persistence, median, Ridge, XGBoost, calibrated estimate of high-risk-event probability, 10-model bootstrap spread, deterministic SHAP explanations, explicit abstention, serialized artifacts, and five required scenario types are implemented.
...
- The Next.js app provides the event queue, case workspace, and model laboratory, uses local frozen JSON when the Python API is unavailable, and makes reveal-outcome the only route for post-T−48 truth.
...
- **M3 / AI-improvement claim:** on the frozen real-data hold-out, the selected conservative ensemble reduces MAE from 5.080 to 3.052 log-risk units but ties persistence on ESA-style loss at 0.167. The product correctly disables the claim that it beats persistence until both required metrics improve.
