...
Use the uncertain case for the 90-second path and the laboratory for longer questions. Keep the language exactly as written around thresholds and results: `−6` is the ESA challenge class, the ranges are model spread, and the model lowers MAE but ties persistence on ESA loss.

## 90-second script

“Collision-risk estimates can change as new tracking observations arrive, but an operator still needs time to prepare. PRISM asks a narrow question: using only conjunction messages available at least 48 hours before closest approach, can we forecast the final reported collision risk?

These five cases come from the real ESA Collision Avoidance Challenge archive. I’ll open ‘Needs a human look.’ The page is frozen before T−48. It shows the current log collision risk, the forecast, the warning probability, and the spread across ten bootstrap models. This spread crosses the configured ESA challenge class, so PRISM does not hide the disagreement—it asks for human review.
...
In the laboratory, the ensemble lowers held-out mean absolute error from 5.080 to 3.053, but it ties persistence on ESA-style high-risk loss. I therefore do not claim that AI wins every required metric. PRISM is an educational forecasting copilot; it never commands a manoeuvre, and a human remains responsible.”
...
The data are the public ESA Collision Avoidance Challenge archive from 2015–2019. The original training ZIP contains 162,634 CDM rows across 13,154 events. PRISM keeps 8,293 events that have both a usable pre-T−48 history and a later labeled outcome. The official challenge test CSV has no final labels, so I use it to verify input compatibility, not to claim model quality.
...
The learned model is XGBoost because this is medium-sized, missing-valued tabular data. I compare it with a constant median, Ridge, and the strongest simple baseline: persistence, which copies the latest pre-T−48 risk forward. Ten bootstrap XGBoost models provide a point forecast and model spread. A separate calibration split maps the forecast to the probability of the ESA challenge high-risk class, `log10(Pc) ≥ −6`. That line is a competition scoring class, not an ISRO operational rule.
...
On 1,659 untouched test events, the selected guarded ensemble reduces MAE from 5.080 to 3.053, or 39.9%. Both it and persistence score 0.167 on ESA-style loss, so the full PRD requirement to beat persistence is not met. The model-spread bands also under-cover: the nominal 90% band contains only 49.2% of outcomes. The interface calls it model spread rather than a calibrated interval.
...
A separate calibration partition fits the warning-probability map without seeing test labels. The bootstrap distribution supplies 50% and 90% spread bands. Their empirical coverage is only 27.8% and 49.2%, so they are explicitly labeled model spread, not statistical confidence intervals. PRISM abstains when the outer band crosses the class threshold, when critical fields are missing, or when model disagreement is excessive.

[Open the uncertain case.]

The initial payload contains no future messages or final outcome. The status strip puts current risk, forecast, both spread bands, and configured warning probability together. The chart marks T−48 and hides the future region. The explanation ranks signed SHAP contributions and then renders deterministic plain language from those verified directions. SHAP tells us why this fitted model moved; it does not prove that a feature physically caused the encounter risk.
...
On the untouched 1,659-event test split, persistence MAE is 5.080 and the selected ensemble MAE is 3.053, a 39.9% reduction. Median absolute error is 0.476 and 50.8% of events fall within half a log unit. The difficult result is the high-risk tail: both selected ensemble and persistence score 0.167 on the ESA-style loss with F2 of 0.361. The acceptance contract requires lower MAE and lower ESA loss, so I keep the AI-superiority claim disabled.
...
The defensible finding is narrow: cutoff-safe learned features reduce average forecast error on this historical archive, while they do not displace persistence for the high-risk objective. PRISM’s contribution is the complete auditable workflow—real-data provenance, leakage-safe evaluation, baseline comparison, calibrated warning view, measured uncertainty limits, explanations, abstention, and offline presentation. It remains an educational research prototype. It never recommends firing thrusters, and any operational response requires qualified human judgment.”

## Likely questions

**Why XGBoost instead of a transformer?** The dataset is structured tabular history summarized per event, contains extensive missingness, and has only 8,293 eligible examples. XGBoost is the stronger complexity baseline here; a sequence model should replace it only after winning on the same frozen splits.
...
**Is 49.2% coverage acceptable for a 90% interval?** No. That result proves the raw bootstrap spread is under-calibrated. The UI labels it model spread and uses it for abstention, while formal conformal calibration remains future work.
