...
Honest metrics and the residual candidate were scored on the frozen 18 August split; the product specification and exhibit UI were left alone.

\section{Results}

Table~\ref{tab:honest} reports the frozen local test with floor slices and intervals. Of 1659 events, 1352 later reports sit at $-30$. Persistence median AE is 0: more than half of reports do not move after 48 hours. Reconstructing $\widehat{y}=\mathrm{risk}+\widehat{y-\mathrm{risk}}$ tests whether anything besides persistence exists. Residual XGBoost reaches test MAE 2.760 ($\Delta=2.319$ $[1.977,2.685]$), close to unguarded level-valued XGBoost (2.809). Floor-excluded MAE remains worse than persistence (7.375 versus 4.073). Wilcoxon $p=0.92$. Both learned models have $F_2=0$. On the validation split used for candidate ranking, unguarded XGBoost is slightly ahead (MAE 2.669 versus 2.671), so the residual model is a stored candidate, not the live exhibit. The mean-error win is still a floor-jump statistic (H2). ESA-style loss still ties at 0.167 for persistence and the selected ensemble (H3). The constant training-set median still beats the selected policy on overall MAE (3.002).
...
Next measurements are a two-part floor model; split-conformal coverage; a frozen official-test evaluation; a dilution-gap probe for H4; and repeated grouped splits so horizon gaps also carry intervals.
